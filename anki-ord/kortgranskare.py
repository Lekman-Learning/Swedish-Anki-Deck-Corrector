...
VERIFIERARINSTRUKTION = (
    ...
    "KORTEN ANVÄNDER TRE NIVÅER AV SYNONYM, och de ska dömas olika. "
    "Utan tecken = utbytbart ord, kräver att ordboken själv pekar ut det. "
    "`≈ ord` = närmaste ord som finns, kräver också ordbokskälla, ett enda ord "
    "efter tecknet. `≈≈ ord` = KATEGORIN ordet tillhör, alltså medvetet ett "
    "ÖVERORDNAT begrepp -- den nivån kräver INGEN källa och ska INTE prövas som "
    "synonym, eftersom den bara komprimerar kortets egen, redan granskade "
    "definition. Att `≈≈ jordart` på kortet `morän` är överordnat är alltså precis "
    "vad nivån betyder, inte ett fel; underkänn en `≈≈` bara om kategorin är "
    "SAKLIGT FEL eller så vid att den inte säger något alls. "
    "EN TOM SYNONYMRAD är rätt svar BARA för idiom och ordled, där hela uttrycket "
    "bär betydelsen. Ett ENSKILT ORD ska ha minst en rad ifylld på någon av de tre "
    "nivåerna -- saknas den är det ett fel. De "
    "flesta ord har ingen sann synonym -- mätt över 828 uppslag saknar 69 % belägg i "
    "SO och SAOL, och det är just därför nivå `≈≈` finns. Decket pluggas dessutom mot HP-provets ORD-del, som ÄR ett "
